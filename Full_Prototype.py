...
signal_duration = 7  # Change signal every 10 seconds
...
class Vehicle:
    def __init__(self, x, y, vehicle_type, direction, index):
        ...
        if vehicle_type in ["Emergency"]:
            self.priority = 999 # Priority for EVs and AVs
        elif vehicle_type in ["EV"] and self.battery < 20:
            self.priority = 3
    ...
    def move(self, vehicles):
        ...
        if self.direction == "right" and self.x >= 520:
            self.has_crossed_junction = True
        elif self.direction == "left" and self.x <= 460:
            self.has_crossed_junction = True
        elif self.direction == "down" and self.y >= 380:
            self.has_crossed_junction = True
        elif self.direction == "up" and self.y <= 300:
            self.has_crossed_junction = True
    ...
    def update_position(self, vehicles):
        ...
        global e_priority, w_priority, n_priority, s_priority
...
def spawn_new_vehicles(vehicles,index):
    ...
    if len(vehicles) < MAX_VEHICLES:  # Only spawn if the vehicle count is less than MAX_VEHICLES
        ...
        for _ in range(1):
            ...
            vehicle_type = random.choices(["Emergency", "EV", "AV", "Normal"], weights = [1, 3, 3, 5])[0]
...
n_priority = 0
s_priority = 0
singal_rep={"North" : 1, "South" : 1, "East" : 1, "West" : 1}

def optimize_traffic_flow(vehicles):
    """Adjust traffic signals based on the number of EVs and AVs in each direction."""
    global signal_states, signal_timer ,n_priority, s_priority, e_priority, w_priority, singal_rep

    # Count priority vehicles in each direction
    if(singal_rep["North"]==0 and singal_rep["South"]==0 and singal_rep["East"]==0 and singal_rep["West"]==0):
        singal_rep={"North" : 0, "South" : 0, "East" : 0, "West" : 0}
    print(singal_rep)
    ...
    for v in vehicles:
        ...
        if n_priority >= max(s_priority, e_priority, w_priority)  and time.time() - signal_timer > signal_duration and (singal_rep["North"]!=0 or n_priority>=9999):  # Threshold for switching
        ...
        elif s_priority >= max(n_priority, e_priority, w_priority)  and time.time() - signal_timer > signal_duration and (singal_rep["South"]!=0 or n_priority>=9999):
        ...
        elif e_priority >= max(s_priority, n_priority, w_priority)  and time.time() - signal_timer > signal_duration and (singal_rep["East"]!=0 or n_priority>=9999):
        ...
        elif w_priority >= max(s_priority, e_priority, n_priority)  and time.time() - signal_timer > signal_duration and (singal_rep["West"]!=0 or n_priority>=9999):
    ...
    for vehicle in vehicles:
        ...
        if vehicle.direction == "right" and vehicle.has_crossed_junction and vehicle.has_crossed == False:
            e_priority-=vehicle.priority
            
        elif vehicle.direction == "left" and vehicle.has_crossed_junction and vehicle.has_crossed == False:
            w_priority-=vehicle.priority
        
        elif vehicle.direction == "down" and vehicle.has_crossed_junction and vehicle.has_crossed == False:
            s_priority-=vehicle.priority
            
        elif vehicle.direction == "up" and vehicle.has_crossed_junction and vehicle.has_crossed == False:
...
while running:
    ...
    for i, (direction, color) in enumerate(signal_states.items()):
        ...
        if direction == "North":
            ...
            pygame.draw.circle(screen, YELLOW if time.time() - signal_timer > signal_duration - 1 else GRAY, (x, y + 40), 15) 
        elif direction == "East":
            x, y = (325, 180)
            light_color = GREEN if color == GREEN else RED
            pygame.draw.circle(screen, light_color, (x, y), 15)
            ...
            pygame.draw.circle(screen, YELLOW if time.time() - signal_timer > signal_duration - 1 else GRAY, (a, b + 40), 15)
        elif direction == "West" :
            a, b = (640, 500)
            light_color = GREEN if color == GREEN else RED
            pygame.draw.circle(screen, light_color, (a, b), 15)
